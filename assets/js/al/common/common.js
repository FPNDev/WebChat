...
var ajax = {
    ...
    api: function api(url, data, callback) {
        return ajax.json('/methods/'+url+'.php', data, callback);
    },

    json: function json(url, data, callback) {
        ...
        xhr.addEventListener('readystatechange', function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                try {
                    ...
                    return notifier.message(2, 'Произошла ошибка во время парсинга JSON.<br>Подробности в консоли.');
                }
                if(json.act) eval(json.act);
                ajax.wjson(json);
                callback && callback(json, this);
            }
        });
        this.addRH(xhr);
        xhr.send();
    ...
    plain: function plain(url, callback) {
        var async = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];

        var xhr = new XMLHttpRequest();
        xhr.open('POST', url, async);
        xhr.addEventListener('readystatechange', function () {
            if (this.readyState == 4 && this.status == 200) callback && callback(this.response, this);
        });
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        this.addRH(xhr);
        xhr.send(ajax.serialize({
            referer_view: nu(VIEW_NAME) ? VIEW_NAME : '',
            direction: nav.pUUID > nav.UUID ? -1 : 1,
            ajax: 1
        }));
        return xhr;
    },
    ...
    post: function post(url, data, callback) {
        ...
        var headers = arguments.length <= 4 || arguments[4] === undefined ? false : arguments[4];
        ...
        if (!isObject(headers) || !headers['Content-Type']) headers = { 'Content-Type': 'application/x-www-form-urlencoded' };
    ...
    block: function block(url, data, _block, callback) {
        var async = arguments.length <= 4 || arguments[4] === undefined ? true : arguments[4];
        var headers = arguments.length <= 5 || arguments[5] === undefined ? false : arguments[5];

        ajax.post(url, data, function (d, x) {
            sel(_block, false).innerHTML = sel(_block, ce('html', { innerHTML: d }), false).innerHTML;
            callback && callback(d, x);
        }, async, headers);
    },
    ...
    form: {
        serialize: function serialize(obj) {
            var data = {},
                objs = sel('input, select, textarea', obj);
            objs.each(function (el) {
                var n = el.a('name'),
                    v = el.value;
                if (el.a('type') == 'radio' && !el.checked) return;
                if (n) data[n] = v;
            });
            return ajax.serialize(data);
        },

        send: function send(form, callback) {
            var async = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];
            ...
            switch (method) {
                case 'get':
                    return ajax.get(ajax.form.getlnk(form), callback, async);
                case 'post':
                    return ajax.post(action, ajax.form.serialize(form), callback, async, {});
            }

            return false;
        },

        json: function send(form, callback, async) {
            return ajax.form.send(form, function(data, x) {
                var json = {};
                try {
                ...
                ajax.wjson(json);
                callback && callback(json, x);
            }, async);
        },

        getlnk: function getlnk(form) {
            return (form.getAttribute('action') || location.pathname) + '?' + ajax.form.serialize(form);
        }
    },

    wjson: function(jd) {
        var st = ne(jd.success) && nu(jd.success.type) ? jd.success : false,
            wt = ne(jd.warning) && nu(jd.warning.type) ? jd.warning : false,
            et = ne(jd.error) && nu(jd.error.type) ? jd.error : false;
        ...
        switch(nt.type || '') {
            ...
            case 'notifier':
                ...
                break;
            case 'inputs':
                var fo = sel('#'+nt.form, false);
                if(!fo) break;
                var ik = Object.keys(nt.errors),
                    objs = sel('input, select, textarea', fo);
                objs.each(function (el) {
                    var nl = el.nextElementSibling;
                    if(!nl || !nl.matches('label')) nl = false;
                    if(ik.indexOf(el.name) !== -1) {
                        !el.classList.remove('error') && nl && nl.classList.remove('error');
                        setTimeout(function() {
                            !el.classList.add('error') && nl && nl.classList.add('error');
                        }, 1);
                    }
                });
                break;
...
Object.assign = function() {
    ...
    return arguments.reduce(function (r, o) {
...
function sel(selector) {
    var arg1 = arguments.length <= 1 || arguments[1] === undefined ? true : arguments[1];
    var arg2 = arguments.length <= 2 || arguments[2] === undefined ? document : arguments[2];

    if ((typeof selector === 'undefined' ? 'undefined' : _typeof(selector)) == 'object') return selector;
    var context = arg2,
        all = arg1;
    if ((typeof arg1 === 'undefined' ? 'undefined' : _typeof(arg1)) == 'object') context = arg1, arg2 = arg2 ? true : false, all = arg2;
    selector = selector.trim();
    if (context === document && !all && /^\s*#[\w_-]+\s*$/g.test(selector)) {
        return document.getElementById(selector.substr(1));
    }
    if (/^[a-z]+$/gi.test(selector)) {
        return all ? context.getElementsByTagName(selector) : context.getElementsByTagName(selector)[0];
    }
    if (/^\.[\w_-]_$/g.test(selector)) {
        return all ? context.getElementsByClassName(selector) : context.getElementsByTagName(selector)[0];
    }
    if (/^\[name="[^"]+"\]$/g.test(selector) && context === document) {
        return all ? document.getElementsByName(selector) : context.getElementsByName(selector)[0];
    }
    if (all) return context.querySelectorAll(selector);else return context.querySelector(selector);
};

function createElementFunc(functionName, func) {
    var allowMany = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];

    if (!Array.prototype.isPrototypeOf(functionName)) {
        Element.prototype[functionName] = Document.prototype[functionName] = Window.prototype[functionName] = func;
        if (allowMany) NodeList.prototype[functionName] = HTMLCollection.prototype[functionName] = func;
    } else {
        functionName.forEach(function (fName) {
            Element.prototype[fName] = Document.prototype[fName] = Window.prototype[fName] = func;
            if (allowMany) NodeList.prototype[fName] = HTMLCollection.prototype[fName] = func;
        });
    }
    return true;
}

createElementFunc('queryParent', function (selector) {
    var p = this;
    while (p = p.parentNode) {
        if (Element.prototype.isPrototypeOf(p) && p.matches(selector)) return p;
    }return false;
});

createElementFunc('a', function (property, value) {
    if (!nu(value)) {
        var tga = ne(this.getAttribute) ? this.getAttribute(property) : undefined;
        return !nu(tga) ? this[0].getAttribute ? this[0].getAttribute(property) : undefined : tga;
    }
    if (NodeList.prototype.isPrototypeOf(this) || HTMLCollection.prototype.isPrototypeOf(this)) this.each(function (v) {
        value === null ? v.setAttribute(property, value) : v.removeAttribute(property);
    });else value !== null ? this.setAttribute(property, value) : this.removeAttribute(property);
    return this;
});

createElementFunc('css', function (property, value) {
    if (typeof value == 'undefined') return typeof this.style[property] == 'undefined' ? this[0].style[property] : this.style[property];
    if (NodeList.prototype.isPrototypeOf(this) || HTMLCollection.prototype.isPrototypeOf(this)) this.each(function (v) {
        v.style[property] = value;
    });else this.style[property] = value;
    return this;
});

createElementFunc('offset', function (rel) {
    rel = rel || document.body;
    var a = this.getBoundingClientRect ? this.getBoundingClientRect() : { top: 0, left: 0 };
    var b = rel.getBoundingClientRect ? rel.getBoundingClientRect() : { top: 0, left: 0 };
    return { top: a.top - b.top + (document.scrollTop || 0) - (document.clientTop || 0), left: a.left - b.left + (document.scrollLeft || 0) - (document.clientLeft || 0) };
});

createElementFunc('wo', function () {
...
createElementFunc('isChildOf', function (parent) {
    var node = this.parentNode;
    while (node != null) {
        if (node == parent || typeof parent == 'string' && node.matches && node.matches(parent)) {
            return true;
        }
...
createElementFunc(['insertAfter', 'ia'], function (referenceNode) {
    referenceNode.parentNode.insertBefore(this, referenceNode.nextSibling);
});

createElementFunc('ib', function (referenceNode) {
    referenceNode.parentNode.insertBefore(this, referenceNode);
});

createElementFunc('prepend', function(referenceNode) {
    this.insertBefore(referenceNode, this.firstChild);
});

createElementFunc('runEvent', function (name, parent, params) {
    if (!Object.prototype.isPrototypeOf(parent)) return;
    var ev = new CustomEvent(name);
    if(nu(parent)) {
        (nu(params) ? params : events.getParams(name)).forEach(function (v) {
            if (nu(parent[v])) ev[v] = parent[v];
        });
    }
...
function CEvent(eventName, eventParams) {
...
var events = {
    initialize: function initialize(eventName, eventParams, variables) {
        this.evs[eventName] = new CEvent(eventName, eventParams, variables);
        return true;
    },
    set: function set(eventName, varName, varValue) {
        this.evs[eventName].set(varName, varValue);
        return true;
    },
    get: function get(eventName, varName) {
        return this.evs[eventName].get(varName);
    },
    getParams: function getParams(eventName) {
        if (this.evs[eventName]) return this.evs[eventName].eventParams;else return [];
    },
    evs: {}
};
...
function initialize() {
    init.go();
    sel('#self_destruct').each(function (v) {
        v.remove();
    });
    sel('#modal_wrap').bind('click', function (ev) {
        var p = ev.path || (ev.composedPath && ev.composedPath()) || [];
        if (p[0].matches('#modal_wrap')) modal.hide();
    }, false, 'modal');
}

var init = {
    view: function view(_view, func) {
        var exclude = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];

        if (_view.forEach) {
            _view.forEach(function (v) {
                var funcs = init.funcs[exclude ? 'exclude' : 'include'][v];
                if (!funcs) funcs = [];
                funcs.push(func);
                init.funcs[exclude ? 'exclude' : 'include'][_view] = funcs;
            });
        } else {
            var funcs = init.funcs[exclude ? 'exclude' : 'include'][_view];
            if (!funcs) funcs = [];
            funcs.push(func);
            init.funcs[exclude ? 'exclude' : 'include'][_view] = funcs;
        }
    ...
    go: function go() {
        if (typeof VIEW_NAME == 'undefined') return true;
        for (var k in this.funcs.include) {
            var v = this.funcs.include[k];
            if (VIEW_NAME == k) {
        ...
        for (var _k in this.funcs.exclude) {
            var _v = this.funcs.exclude[_k];
            if (VIEW_NAME != _k) {
                _v.forEach(function (f) {
                    try {
                        f();
                    } catch (e) {}
                });
    ...
    isIncludedFunction: function isIncludedFunction(func) {
        ...
        for (var k in this.funcs.include) {
            ...
            if (VIEW_NAME == k) {
                v.forEach(function (f) {
                    if (f.toString() == func.toString()) included = true;
                });
            }
        };
        for (var _k2 in this.funcs.exclude) {
            var _v2 = this.funcs.exclude[_k2];
            if (VIEW_NAME != _k2) {
                _v2.forEach(function (f) {
                    if (f.toString() == func.toString()) included = true;
                });
            }
        };
    ...
    funcs: { include: {}, exclude: {} }
};

var listeners = {
    getListeners: function getListeners(el) {
        var iterator = -1;
        if ((iterator = this.listenerElements.indexOf(el)) == -1) {
            this.listenerElements.push(el);
            this.listenerEvents.push([]);
            iterator = this.listenerEvents.length - 1;
        }
        return this.listenerEvents[iterator];
    }, listenerElements: [], listenerEvents: []
};

createElementFunc('on', function (ev, callback) {
    var bool = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];

    var evs = [];
    if ((evs = ev.split(' ')).length > 1) {
        evs.each(function (ev) {
            this.on(ev, callback, bool);
        });
        return this;
    }
    if (NodeList.prototype.isPrototypeOf(this) || HTMLCollection.prototype.isPrototypeOf(this)) {
        this.each(function (v) {
            var l = listeners.getListeners(v);
            v.addEventListener(ev, callback, bool);
            l.push({ el: v, type: ev, listener: callback, ident: '', bool: bool, active: true });
        });
    } else {
        var l = listeners.getListeners(this);
        this.addEventListener(ev, callback, bool);
        l.push({ el: this, type: ev, listener: callback, ident: '', bool: bool, active: true });
    }

    return this;
});

createElementFunc('bind', function (ev, callback) {
    var bool = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];
    var ident = arguments.length <= 3 || arguments[3] === undefined ? '' : arguments[3];

    var evs = [];
    if ((evs = ev.split(' ')).length > 1) {
        evs.each(function (ev) {
            this.bind(ev, callback, bool, ident);
        });
        return this;
    }
    if (typeof bool != 'boolean' && !ident) ident = bool, bool = true;
    this.off(ev, ident);
    if (NodeList.prototype.isPrototypeOf(this) || HTMLCollection.prototype.isPrototypeOf(this)) {
        this.each(function (v) {
            var l = listeners.getListeners(v);
            v.addEventListener(ev, callback, bool);
            l.push({ el: v, type: ev, listener: callback, ident: ident, bool: bool, active: true });
        });
    } else {
        var l = listeners.getListeners(this);
        this.addEventListener(ev, callback, bool);
        l.push({ el: this, type: ev, listener: callback, ident: ident, bool: bool, active: true });
    }

    return this;
});

createElementFunc('off', function (ev) {
    ...
    if (NodeList.prototype.isPrototypeOf(this) || HTMLCollection.prototype.isPrototypeOf(this)) {
        this.each(function (v) {
            var lg = listeners.getListeners(v);
            lg.forEach(function (l, k) {
                if (l.type == ev && l.ident == ident && l.active) {
                    l.el.removeEventListener(ev, l.listener, l.bool);
                    l.active = false;
                }
            });
        });
    } else {
        var lg = listeners.getListeners(this);
        lg.forEach(function (l, k) {
            if (l.type == ev && l.ident == ident && l.active) {
                l.el.removeEventListener(ev, l.listener, l.bool);
                l.active = false;
...
var modal = {
    show: function show(loc, data, ev) {
        if (Event.prototype.isPrototypeOf(data) && !ev) ev = data, data = false;
        if (ev) ev.preventDefault();
        if (typeof loc != 'string') loc = loc.getAttribute('href');
        sel('*').css('cursor', 'progress');
        sel('#modal_bg', false).css('visibility', 'visible').css('opacity', 1);
        if (!Object.prototype.isPrototypeOf(data)) data = {};
        data.ajax = 1;
        ajax.post(loc, ajax.serialize(data), function (data) {
            data = JSON.parse(data);
            var files = data.scripts.concat(data.styles);

            stManager.add(files, function () {
                sel('#modal_wrap', false).css('visibility', 'visible').css('opacity', 1).innerHTML = data.body;
                sel('*').css("cursor", "");
                scrollDisable();
                modal.active = true;
                onBodyResize();
    ...
    hide: function hide() {
        if (modal.active) {
            scrollEnable();
            sel('#modal_bg', false).removeAttribute('style'), sel('#modal_wrap', false).removeAttribute('style');
            modal.active = false;
        }return true;
    },
...
document.on('keydown', function (ev) {
    if ((ev.keyCode || ev.which) == 27) modal.hide();
});

var nav = {

    go: function(that, event, params) {
        if(event) event.preventDefault();
        if(!nu(params) || !params) params = {};
        if(!nu(params.wrapper)) params.wrapper = '#page-wrapper';
        if(!nu(params.loader)) params.loader = false;
        var href = that;
        if(typeof that != 'string') href = that.href || that.getAttribute('href');

        if(this.isExternalLink(href)) return window.location.href = href; 

        if(!ne(history.state)) history.replaceState({UUID: 0}, null, location.href);

        if(params.loader && isObject(load)) load.start();

        this.lastUrl = location.pathname + location.search;
        sel('*').css("cursor", "progress");
        if(this.xhr) this.xhr.abort();
        var xhr = this.xhr = ajax.plain(href, function(data, xhr) {
            this.pUUID = this.UUID;
            if(nu(params.UUID)) this.UUID = params.UUID;
            else this.UUID += 1;
            params.UUID = this.UUID;
            if(href != location.href && href != this.lastUrl) history.pushState({UUID: this.UUID}, null, xhr.responseURL);

            document.runEvent('beforeNav', {}, []);
            data = JSON.parse(data);
            document.title = data.title;

            sel('meta', document.head).each(function(v) {
                v.remove();
            });
            
            data.meta.forEach(function(v) {
            ...
            var files = {css: data.styles, js: data.scripts};
            nav.files = files;
            stManager.add(files, function() {
                var w = sel(params.wrapper, false),
                    nw = w.cloneNode(false),
                    ww = w.offsetWidth,
                    wh = w.offsetHeight,
                    wZ = parseInt(getComputedStyle(w).zIndex);
                w.a('style', '');
                w.className = '';
                w.style.position = 'absolute';
                w.style.top = 0;
                w.style.left = 0;
                nw.innerHTML = data.body;
                nw.className = '';
                nw.ib(w);
                w.style.height = nw.style.height = nw.offsetHeight + 'px';
                w.style.width = nw.style.width = nw.offsetWidth + 'px';
                w.style.overflow = 'hidden';
                var onw = nw.wo(), ow = w.wo();
                if(onw.top != ow.top) w.style.marginTop = (onw.top - ow.top) + 'px';
                if(onw.left != ow.left) w.style.marginLeft = (onw.left - ow.left) + 'px';
                nw.css('display', 'none');
                if(ne(data.anim) && nu(sAnim)) {
                    var nzi = ne(data.anim.zIndex) ? -1 : 1;
                    w.style.zIndex = nzi;
                    var ae = false;
                    if(ne(data.anim.old)) {
                        ae = true;
                        w.a('sAnim', data.anim.old.name);
                        if(ne(data.anim.old.duration)) w.a('sDur', data.anim.old.duration);
                        if(ne(data.anim.old.delay)) w.a('sDelay', data.anim.old.delay);
                    }
                    if(ne(data.anim.new)) {
                        ae = true;
                        nw.a('sAnim', data.anim.new.name);
                        if(ne(data.anim.new.duration)) nw.a('sDur', data.anim.new.duration);
                        if(ne(data.anim.new.delay)) nw.a('sDelay', data.anim.new.delay);
                    }
                    nw.style = '';
                    sAnim.transform(w); sAnim.transform(nw);
                    var onw = nw.wo(), ow = w.wo();
                    if(ae) {
                        var csw = getComputedStyle(w),
                            csnw = getComputedStyle(nw),
                            lae = (parseFloat(csnw.animationDuration) || 0) + (parseFloat(csnw.animationDelay) || 0) > (parseFloat(csw.animationDuration) || 0) + (parseFloat(csw.animationDelay) || 0) ? nw : w;
                        if((lae == nw && !csnw.animationName) || (lae == w && !csw.animationName)) {
                            w.remove();
                            stManager.remove(nav.files);
                            if(nu(params.callback)) params.callback && params.callback();
                        } else {
                            var kk = nav.animEndID++;
                            lae.bind('animationend webkitAnimationEnd mozAnimationEnd DOMNodeRemoved', function(ev) {
                                if(ev.target != lae) return;
                                lae.off('animationend webkitAnimationEnd mozAnimationEnd DOMNodeRemoved', 'navAnimationEnd_'+kk);
                                w.remove();
                                stManager.remove(nav.files);
                                if(nu(params.callback)) params.callback && params.callback();
                            }, false, 'navAnimationEnd_'+kk);
                        }
                    } else {
                        w.remove();
                        stManager.remove(nav.files);
                ...
                if(params.loader && isObject(load)) load.end();
            });
        });
        if(params.loader && isObject(load)) {
    ...
    goWithProgress: function(that, event, params) {
        return nav.go(that, event, Object.assign({}, params, { loader: true }));
    },
    ...
    isExternalLink: function(url) {
        ...
        return domain(url) ? domain(location.href)[1] !== domain(url)[1] : false;
    }, lastUrl: location.pathname + location.search, UUID: 0, pUUID: 0, xhr: 0, animEndID: 0, files: []
...
var load = {
    ...
    start: function() {
        if(!sel('.loading-bar', false)) {
            sel('body', false).insertAdjacentHTML('beforeend', '<div class="loading-bar" style="width: 10%"></div>');
        }
    },
    set: function(amount) {
        if(sel('.loading-bar', false)) {
            amount = amount || this.get();
            sel('.loading-bar', false).css('width', amount + '%');
        }
    },
    end: function() {
        if(sel('.loading-bar', false)) {
            sel('.loading-bar', false).css('width', '100%');
            sel('.loading-bar', false).css('opacity', 0);
            sel('.loading-bar', false).on('transitionend', function() {
                this.remove();
            });
        }
    },
    get: function() {
        var amount = (sel('.loading-bar', false).offsetWidth / sel('body', false).offsetWidth) * 100;
        return amount;
    }
...
var notifier = {
    message: function message(mode, _message) {
        var time = arguments.length <= 2 || arguments[2] === undefined ? 4000 : arguments[2];

        if (!nu(_message) || !_message) return;
        if (_message.length > 500) _message = _message.substr(0, 497) + '...';
        var nb = sel('.notifier-bar', false);
        if (!nb) nb = this.createNotifierBar();
        var cName = 'success';
        switch (mode) {
            case 1:
                cName = 'warning';
        ...
        var notify = this.tpl(_message, cName);
        nb.prepend(notify);
        var csn = getComputedStyle(notify),
            nt = csn.transitionProperty == 'opacity';

        if (nt) notify.css('opacity', 1);

        var notifyRemoveTimeout = setTimeout(function () {
            if (nu(notify)) {
                if (nt) {
                    notify.css('opacity', 0);
                    notify.bind('transitionend', function () {
                        this.off('transitionend', 'notifyTransitionEnd');
                        this.remove && this.remove();
                    }, true, 'notifyTransitionEnd');
                } else notify.remove();
            }
        }, time);
        var nta = sel('.notify', nb);
        if (nta.length == this.maxNotify + 1) nta[nta.length - 1].remove();
    },
    createNotifierBar: function createNotifierBar() {
        var nb = ce('div', { className: 'notifier-bar' });
        document.body.prepend(nb);
        return nb;
    },

    json: function json(obj) {
        if(!nu(obj) || !isObject(obj)) return false;
        
        var mh = obj.error || obj.warning || obj.success,
            t = mh === obj.success ? 0 : (mh === obj.warning ? 1 : 2);

        return ne(mh) && mh.msg && notifier.message(t, mh.msg);
    },

    tpl: function tpl(message) {
        var className = arguments.length <= 1 || arguments[1] === undefined ? 'success' : arguments[1];

        var notify = ce('div', { className: 'notify' });
        notify.classList.add(className);
        var nm = ce('div', { className: 'notify-message', innerHTML: message }),
            ni = ce('div', { className: 'notify-icon' }),
            ico = ce('i', { className: 'material-icons' });
        switch (className) {
            case 'success':
                ico.innerHTML = 'check_circle';
                break;
            case 'warning':
                ico.innerHTML = 'warning';
                break;
            case 'error':
                ico.innerHTML = 'error';
                break;
        ...
        ni.appendChild(ico);
        notify.appendChild(ni);
        notify.appendChild(nm);
        return notify;
    }, maxNotify: 4
};
...
function fpntpl(id, data) {
    var html = fpntemplates[id] || '';
    for (var k in data) {
        html = html.replace(new RegExp(RegExp.escape('[[' + k + ']]'), 'g'), data[k]);
    }
    return html;
}

function tplinit() {
    sel('.fpntpl').each(function (v) {
        fpntemplates[v.id] = v.innerHTML;
        v.remove();
    });
}
...
var eventProps = ['eventName', 'altKey', 'bubbles', 'button', 'buttons', 'cancelBubble', 'cancelable', 'clientX', 'clientY', 'composed', 'ctrlKey', 'currentTarget', 'defaultPrevented', 'detail', 'eventPhase', 'fromElement', 'isTrusted', 'layerX', 'layerY', 'metaKey', 'movementX', 'movementY', 'offsetX', 'offsetY', 'pageX', 'pageY', 'path', 'relatedTarget', 'returnValue', 'screenX', 'screenY', 'shiftKey', 'sourceCapabilities', 'target', 'timeStamp', 'toElement', 'type', 'view', 'which', 'x', 'y'];
...
browser.ios = browser.iphone || browser.ipod;
browser.hls = ['maybe', 'probably'].indexOf(document.createElement('video').canPlayType('application/vnd.apple.mpegURL')) != -1;

screen.unlockOrientation = screen.unlockOrientation || screen.mozUnlockOrientation || screen.msUnlockOrientation || screen.orientation && screen.orientation.unlock.bind(screen.orientation);
screen.lockOrientation = screen.lockOrientation || screen.mozLockOrientation || screen.msLockOrientation || screen.orientation && screen.orientation.lock.bind(screen.orientation);
...
createElementFunc('each', function (callback) {
    var reverse = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];

    if (typeof callback != 'function') return this;
    var obj = this;
    if (NodeList.prototype.isPrototypeOf(obj) || HTMLCollection.prototype.isPrototypeOf(obj) || Array.prototype.isPrototypeOf(obj)) {
        var oobj = Object.assign({}, obj);
        if (!reverse) Object.keys(oobj).forEach(function (k) {
            if(k == 'length') return;
            if (nu(oobj[k])) callback(oobj[k], k);
        });else {
            Object.keys(oobj).reverse().forEach(function (k) {
                if (nu(oobj[k])) callback(oobj[k], k);
            });
        }
    } else callback(obj, 0);
...
Array.prototype.each = Element.prototype.each;
...
function ce(el) {
    ...
    if (!isObject(params) || isArray(params)) params = {};
...
function bound(el, parent) {
    var woe = el.wo(),
        wop = parent.wo();
    return woe.top > wop.top && woe.left > wop.left && woe.top < wop.top + parent.offsetHeight && woe.left < wop.left + parent.offsetWidth;
}

function cbound(x, y, parent) {
    var wop = parent.wo();
    return y > wop.top && x > wop.left && y < wop.top + parent.offsetHeight && x < wop.left + parent.offsetWidth;
}

function holding(keyCode) {
    if (keysHolding[keyCode]) return true;
    return false;
}

function ne(variable) {
    return typeof variable != 'undefined' && variable;
}
function nu(variable) {
    return typeof variable != 'undefined';
}
function isArray(variable) {
    return Array.prototype.isPrototypeOf(variable);
}
function isObject(variable) {
    return Object.prototype.isPrototypeOf(variable) && !Array.prototype.isPrototypeOf(variable);
}
function isFormData(variable) {
    return FormData.prototype.isPrototypeOf(variable);
}
function isElement(variable) {
    return Element.prototype.isPrototypeOf(variable);
}
...
document.bind('keydown', function (ev) {
    keysHolding[ev.keyCode || ev.which] = true;
}, false, 'holding');

document.bind('keyup', function (ev) {
    keysHolding[ev.keyCode || ev.which] = false;
}, false, 'holding');

document.bind('DOMContentLoaded', function () {
    scrollElement = sel('#scroll_fix_wrap', false) || document.body;
    initialize();
});

function getSpecialDate(el, time) {
    el.specialDateFunc = function (time) {
        var t = Date.now();
        var cur = new Date(t);
        ...
        time /= 1000;
        var diff = t - time;
        var year = cur.getFullYear() - past.getFullYear();
        var day = Math.floor(diff / 86400);
        if (cur.getHours() < past.getHours()) day++;
        if (year && day > 2) {
            var str = past.toLocaleString('ru-RU', { day: '2-digit', month: 'long' }) + ' ' + past.getFullYear() + ', в ' + past.getHours().pad(2) + ':' + past.getMinutes().pad(2);
            return this.innerHTML = str;
        }
        var month = Math.floor(diff / 2592000);
        if (cur.getDate() < past.getDate()) month++;
        if (month || day > 2) {
            var _str = past.toLocaleString('ru-RU', { day: '2-digit', month: 'long' }) + ', ' + past.getHours().pad(2) + ':' + past.getMinutes().pad(2);
            return this.innerHTML = _str;
        }
        var hour = Math.floor(diff / 3600);
        if (day || hour > 4) {
            var _str2 = ' в ' + past.getHours().pad(2) + ':' + past.getMinutes().pad(2);
            switch (day) {
                case 0:
                    _str2 = 'Сегодня' + _str2;break;
                case 1:
                    _str2 = 'Вчера' + _str2;break;
                case 2:
                    _str2 = 'Позавчера' + _str2;break;
            }return this.innerHTML = _str2;
        }
        if (hour) {
            var _str3 = '';
            if (hour == 1) _str3 = 'Час назад';else if (hour == 2) _str3 = 'Два часа назад';else if (hour == 3) _str3 = 'Три часа назад';else if (hour == 4) _str3 = 'Четыре часа назад';
            return this.innerHTML = _str3;
        }
        var minutes = Math.floor(diff / 60);
        if (minutes) {
            var _str4 = '';
            if (minutes < 6) {
                if (minutes == 1) _str4 = 'Минуту назад';else if (minutes == 2) _str4 = 'Две минуты назад';else if (minutes == 3) _str4 = 'Три минуты назад';else if (minutes == 4) _str4 = 'Четыре минуты назад';else if (minutes == 5) _str4 = 'Пять минут назад';
            } else {
                var c = minutes.toString();
                var c2 = c.substr(Math.max(c.length - 2, 0), c.length);c = c[c.length - 1];
                if (c == '0' || c == '5' || c == '6' || c == '7' || c == '8' || c == '9' || c == '1' && c2 == '11') _str4 = minutes + ' минут назад';else if (c == '1' && c2 != '11') _str4 = minutes + ' минуту назад';else if (c == '2' || c == '3' || c == '4') {
                    if (c2[0] == '1') _str4 = minutes + ' минут назад';else _str4 = minutes + ' минуты назад';
                }
            }
            return this.innerHTML = _str4;
        }
        var seconds = Math.floor(diff);
        if (seconds) {
            var _str5 = '';
            if (seconds < 6) {
                if (seconds == 1) _str5 = 'Cекунду назад';else if (seconds == 2) _str5 = 'Две секунды назад';else if (seconds == 3) _str5 = 'Три секунды назад';else if (seconds == 4) _str5 = 'Четыре секунды назад';else if (seconds == 5) _str5 = 'Пять секунд назад';
            } else {
                var _c = seconds.toString();
                var _c2 = _c.substr(Math.max(_c.length - 2, 0), _c.length);_c = _c[_c.length - 1];
                if (_c == '0' || _c == '5' || _c == '6' || _c == '7' || _c == '8' || _c == '9' || _c == '1' && _c2 == '11') _str5 = seconds + ' секунд назад';else if (_c == '1' && _c2 != '11') _str5 = seconds + ' секунду назад';else if (_c == '2' || _c == '3' || _c == '4') {
                    if (_c2[0] == '1') _str5 = seconds + ' секунд назад';else _str5 = seconds + ' секунды назад';
                }
            }return this.innerHTML = _str5;
        }return this.innerHTML = 'Только что';
    };
    time = time * 1000;
    if (el.specialDateInterval) clearInterval(el.specialDateInterval);
    el.specialDateFunc(time);
    el.specialDateInterval = setInterval(function () {
        el.specialDateFunc(time);
    }, 5000);
}
...
Number.prototype.format = function(c, t, d){
    var n = this, 
...
var stManager = {
    ...
    find: function find(files, file) {
        var io = isObject(files),
            css = io ? files.css.contains(file) : file.split('?')[0].match(/\.css$/);
        if (css) {
            var link = sel('link');
            var ext = nav.isExternalLink(file);
            if (!ext) file = file.split('?')[0];
            for (var i in link) {
                if (!link[i].getAttribute || !link[i].getAttribute('href')) continue;
                var lnk = link[i].getAttribute('href');
                lnk = ext ? lnk : lnk.split('?')[0];

                if (lnk == file) return true;
            }
            return false;
        } else {
            var script = sel('script');
            var _ext = nav.isExternalLink(file);
            if (!_ext) file = file.split('?')[0];
            for (var _i in script) {
                if (!script[_i].getAttribute || !script[_i].getAttribute('src')) continue;
                var _lnk = script[_i].getAttribute('src');
                _lnk = _ext ? _lnk : _lnk.split('?')[0];
                if (_lnk == file) return true;
            }
            return false;
        }
    },
    ...
    add: function add(files, callback) {
        if (!isObject(files) && !isArray(files)) files = [files];
        var io = isObject(files),
            f = io ? files.css.concat(files.js) : files;
        stManager.done(f, '', callback);
        f.each(function (file) {
            if (stManager.find(files, file)) return stManager.done(f, file, callback);
            var css = io ? files.css.contains(file) : file.split('?')[0].match(/\.css$/);
            if (css) {
                ...
                l.href = file;
                l.onload = function () {
                    stManager.done(f, file, callback);
                };
    ...
    remove: function remove(files) {
        var io = isObject(files),
            f = Array.from(io ? files.css : files),
            al = sel('link:not([static])');
        f = f.map(function (el) {
            return nav.isExternalLink(el) ? el : el.split('?')[0];
        });
        al.each(function (el) {
            var h = el.a('href');
            if (f.indexOf(nav.isExternalLink(h) ? h : h.split('?')[0]) == -1) el.remove();
        });
    },

    done: function done(files, file, callback) {
        if(file) stManager.loaded.push(file);
...
function cookie(name, value) {
    var expires = arguments.length <= 2 || arguments[2] === undefined ? 86400 * 3600 : arguments[2];
...
function storage(name, value) {
    ...
    if (typeof value == 'undefined') {
        value = lstorage.getItem(name);
        if (!value) return null;
        value = JSON.parse(value);
        if (value.time > Date.now()) return value.value;else {
            lstorage.removeItem(name);
            return null;
        }
    }
    lstorage.setItem(name, JSON.stringify({ value: value, time: Date.now() + expires * 1000 }));
    return true;
}

function backup(el, ev) {
    ev.target.src = ev.target.a('backup_src');
} 

var bobs = new MutationObserver(function(ml) {
    for(var m of ml) {
        var i = sel('img[backup_src]:not([initialized])', m.target);
        i.forEach(function(mt) {
            mt.style.visibility = 'hidden';
            mt.a('initialized', '');
            mt.onerror = function(event) {
                backup(mt, event);
                mt.onerror = null;
            }
            mt.onload = function(event) {
                mt.style.visibility = '';
                mt.onerror = null;
                mt.onload = null;
            }
...
bobs.observe(document, {childList: true, subtree: true});